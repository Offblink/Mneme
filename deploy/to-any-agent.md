...
- **`memory_summary.md`** 可以粘贴到项目规则文件（CLAUDE.md / rules.md）中
- **部署策略**：先部署 SKILL.md（确保基础认知），再放置 MEMORY.md（可选），在规则文件中引用 memory_summary.md
...
- `skills/mneme/SKILL.md` — 独立 skill。**无记忆系统的 agent 的最重要文件**——agent 即使没有读到 MEMORY.md，只要部署了这个 skill，就能继承全部核心认知。
...
在对话开始时告诉 agent，或在规则文件中添加：

> 「请先读取项目中的 memory_summary.md 和 MEMORY.md，了解环境和认知框架。」

无记忆系统的平台，将以下内容粘贴到 CLAUDE.md / rules.md / .cursorrules：
...
MEMORY.md and memory_summary.md before the session ends.
```
...
**重要**：每次深度对话、自我反思、操作知识积累后，在会话结束前主动写回 MEMORY.md 和 memory_summary.md。不要假设下一实例会自动继承——它们只能读到被写下的东西。
